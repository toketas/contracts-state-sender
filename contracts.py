...
from utils import ANOTHER_KEY, ANOTHER_VALUE
...
def send_state(key=ANOTHER_KEY, value=ANOTHER_VALUE):
    print('Sending key {}...'.format(key))
    avsc_dir = os.path.dirname(os.path.realpath(__file__))
    value_schema = load(os.path.join(avsc_dir, "ContractState.avsc"))
    key_schema = load(os.path.join(avsc_dir, "Key.avsc"))
    producer = AvroProducer(conf)
    producer.produce(topic='BR02-contracts-state', value=value,
